...
- [Chunk 7 / Task 25 / Step 2] Run `pnpm exec playwright test` locally to confirm the tightened `/contact` (`getByRole("heading", { name: /get in touch|contact/i })`) and `/galleries` (`getByRole("heading", { level: 1 })`) selectors still match. Sandbox lacks a live Next.js server and Playwright browsers.
